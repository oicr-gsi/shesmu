...
export interface ActionFilterCreatedAgo {
  negate?: boolean;
  offset: number;
  type: "createdago";
}

export interface ActionFilterCreated {
  end: number | null;
  negate?: boolean;
  start: number | null;
  type: "created";
...
const timeAccessors: BasicQueryTimeAccessor[] = [
  ...
  {
    rangeType: "created",
    horizonType: "createdago",
    horizon: (query) => query.createdago || null,
    range: (query) => query.created || null,
  },
  {
...
export function nameForBin(name: TimeAgoType | TimeRangeType): string {
  switch (name) {
    ...
    case "created":
    case "createdago":
      return "Time Since Action was First Generated by an Olive";
    case "checked":
    case "checkedago":
