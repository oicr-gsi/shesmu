 ...
 *
 * <p>To create a signature,
 *
 * <ol>
 *   <li>create a public class that implements the {@link Signer} with a public no-arguments
 *       constructor.
 *   <li>create a public class extending this one with a no-arguments constructor that is marked
 *       with {@link MetaInfServices} for {@link SignatureVariable}
 * </ol>
 *
 * @param <T> the signer class to use
 * @param <R> the result value of the signature
 */
public abstract class SignatureVariableForSigner<T, R> extends SignatureVariable {
  ...
  private static final Method METHOD_SIGNER__ADD_VARIABLE =
      new Method(
          "addVariable", Type.VOID_TYPE, new Type[] {A_STRING_TYPE, A_IMYHAT_TYPE, A_OBJECT_TYPE});

  private static final Method METHOD_SIGNER__FINISH =
      new Method("finish", A_OBJECT_TYPE, new Type[] {});

  private final Type outputBuilderType;

  public SignatureVariableForSigner(
      String name,
      Class<? extends Signer<R>> outputBuilderClazz,
      Imyhat type,
      Class<R> returnClazz) {
    ...
    if (!type.javaType().isAssignableFrom(returnClazz)) {
      throw new IllegalArgumentException(
          String.format(
              "Return type of signer %s cannot be assigned to Imyhat %s.",
    ...
    variables.forEach(
        target -> {
          ...
          method.invokeVirtual(
              initialType, new Method(target.name(), target.type().asmType(), new Type[] {}));
