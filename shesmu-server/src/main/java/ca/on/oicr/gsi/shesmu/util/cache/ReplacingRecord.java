...
/**
 * Takes a stream of items and stores them. When updated, it discards the
 * existing items and replaces them all
 */
public final class ReplacingRecord<V> implements Record<Stream<V>> {
	...
	public ReplacingRecord(Owner owner, Updater<Stream<V>> fetcher) {
		...
		List<V> value;
		try {
			value = Optional.ofNullable(fetcher.update(fetchTime)).orElse(Stream.empty()).collect(Collectors.toList());
