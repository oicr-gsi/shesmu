...
public class FetchNodeConstant extends FetchNode {
    ...
    public String renderEcma(EcmaScriptRenderer r) {
        try {
            return String.format("{type:\"constant\",name:%s}", RuntimeSupport.MAPPER.writeValueAsString(target.unaliasedName()));
    ...
    @Override
    public boolean resolveDefinitions(ExpressionCompilerServices expressionCompilerServices, DefinitionRepository nativeDefinitions, Consumer<String> errorHandler) {
        Optional<ConstantDefinition> def = nativeDefinitions.constants().filter(c -> c.name().equals(constantName)).findAny();
        if (def.isPresent()) {
            target = def.get();
            return true;
        } else {
