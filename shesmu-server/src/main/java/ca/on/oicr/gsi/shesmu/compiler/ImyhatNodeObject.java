...
public class ImyhatNodeObject extends ImyhatNode {
	...
	public Imyhat render(Function<String, Imyhat> definedTypes, Consumer<String> errorHandler) {
		...
		if (fieldCounts.entrySet().stream()//
				...
				.peek(e -> errorHandler
						.accept(String.format("Object type has field %s %d times.", e.getKey(), e.getValue())))//
				.count() > 0) {
			return Imyhat.BAD;
		}
		return new Imyhat.ObjectImyhat(types.stream()
				.map(field -> new Pair<>(field.first(), field.second().render(definedTypes, errorHandler))));
	}
