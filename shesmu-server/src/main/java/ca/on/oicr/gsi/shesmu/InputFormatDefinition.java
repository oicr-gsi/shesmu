...
public abstract class InputFormatDefinition {
	private static final ServiceLoader<InputFormatDefinition> LOADER = ServiceLoader.load(InputFormatDefinition.class);
	 ...
	 * @param clazz
	 *            the format requested
	 * @return a stream of available objects or an empty stream if the format is not
	 *         supported
	 */
	...
	/**
	 * Write all the data that would be returned by {@link #input(Class)} for
	 * {@link #itemClass()} to a JSON output stream.
	 */
