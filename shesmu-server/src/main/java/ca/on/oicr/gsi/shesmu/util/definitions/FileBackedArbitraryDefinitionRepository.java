 ...
 * configuration file can create arbitrarily many definitions.
 *
...
public abstract class FileBackedArbitraryDefinitionRepository<T extends FileBackedConfiguration>
  ...
  private class Wrapper implements WatchedFileListener, UserDefiner {
          ...
          new ActionDefinition(
            ...
            public void initialize(GeneratorAdapter methodGen) {
              methodGen.invokeDynamic(
                  fixedName, Type.getMethodDescriptor(Type.getType(clazz)), HANDLE_BSM);
            }
          });
    }
    ...
    public void defineConstant(String name, String description, Imyhat type, Object value) {
      ...
      constants.put(
          name,
          new ConstantDefinition(name, type, description) {

            @Override
            protected void load(GeneratorAdapter methodGen) {
              methodGen.invokeDynamic(
                  fixedName, Type.getMethodDescriptor(type.asmType()), HANDLE_BSM);
            }
          });
    }
    ...
    public <R> void defineConstant(
      ...
      constants.put(
          name,
          new ConstantDefinition(name, returnType.type(), description) {

            @Override
            protected void load(GeneratorAdapter methodGen) {
              methodGen.invokeDynamic(
                  fixedName, Type.getMethodDescriptor(returnType.type().asmType()), HANDLE_BSM);
            }
          });
    }
    ...
    public <A, R> void defineFunction(
      ...
      final String fixedName =
          name + " " + parameterType.type().descriptor() + " " + returnType.type().descriptor();
      register(fixedName, handle);
      functions.put(
          name,
          new FunctionDefinition() {
            ...
            public void render(GeneratorAdapter methodGen) {
              methodGen.invokeDynamic(
                  fixedName,
                  Type.getMethodDescriptor(
                      returnType.type().asmType(), parameterType.type().asmType()),
                  HANDLE_BSM);
            }
          ...
          new FunctionDefinition() {
            ...
            public void render(GeneratorAdapter methodGen) {
              methodGen.invokeDynamic(
                  fixedName,
                  Type.getMethodDescriptor(
                      returnType.type().asmType(),
                      parameter1Type.type().asmType(),
                      parameter2Type.type().asmType()),
                  HANDLE_BSM);
            }
  ...
  private static final Handle HANDLE_BSM =
      new Handle(
          ...
          Type.getType(FileBackedArbitraryDefinitionRepository.class).getInternalName(),
          "bootstrap",
          BSM_DESCRIPTOR,
          false);
  ...
  public static final TypeGlue<String> STRING =
      new TypeGlue<String>() {
  ...
  public FileBackedArbitraryDefinitionRepository(
      Class<T> clazz, String extension, BiFunction<Path, UserDefiner, T> ctor) {
    this.ctor = ctor;
