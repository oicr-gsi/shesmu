...
public final class MonitorArgumentNode {
	public static Parser parse(Parser input, Consumer<MonitorArgumentNode> output) {
		...
		final Parser result = input//
				...
				.whitespace();
		if (result.isGood()) {
			output.accept(new MonitorArgumentNode(input.line(), input.column(), name.get(), expression.get()));
		}
		return result;
	}
	...
	public boolean ensureType(Consumer<String> errorHandler) {
		...
		if (!ok) {
			errorHandler.accept(String.format("%d:%d: Expected argument “%s” to have type string, but got %s.", line,
					column, name, expression.type().name()));
		}
		return ok;
	}
