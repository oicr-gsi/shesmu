...
/**
 * Create a new {@link InputFormatDefinition} that uses an existing input format
 * for the data
 * 
 * This wraps existing input data into a new format. This is useful when
 * changing a data format. Create a wrapper type that looks like the old input
 * format backed by the new input format then create a decorator input format to
 * bridge them.
 *
 * @param <I>
 *            the real (input) type for each “row” to be consumed
 * @param <O>
 *            the decorator (output) type for each “row”, with
 *            {@link ShesmuVariable} annotations
 */
