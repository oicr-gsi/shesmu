...
public class OliveClauseNodeJoin extends OliveClauseNode {
	...
	public OliveClauseNodeJoin(int line, int column, String format, ExpressionNode outerKey, ExpressionNode innerKey) {
		super();
		this.line = line;
		this.column = column;
		this.format = format;
	...
	public Stream<OliveClauseRow> dashboard() {
		return Stream.of(new OliveClauseRow("Join", line, column, true, false, innerInputFormat.baseStreamVariables()//
	...
	public void render(RootBuilder builder, BaseOliveBuilder oliveBuilder,
		...
		final JoinBuilder join = oliveBuilder.join(line, column,innerInputFormat.type(), outerKey.type(), oliveBuilder
	...
	public NameDefinitions resolve(InputFormatDefinition inputFormatDefinition,
		...
		final Set<String> newNames = innerInputFormat.baseStreamVariables()//
				.map(Target::name)//
		...
		boolean ok = outerKey.resolve(defs, errorHandler)
				& innerKey.resolve(defs.replaceStream(innerInputFormat.baseStreamVariables(), true), errorHandler);
