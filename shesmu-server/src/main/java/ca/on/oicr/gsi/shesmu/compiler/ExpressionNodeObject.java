...
public class ExpressionNodeObject extends ExpressionNode {
	...
	public boolean resolve(NameDefinitions defs, Consumer<String> errorHandler) {
		return fields.stream().filter(field -> field.second().resolve(defs, errorHandler)).count() == fields.size();
	}

	@Override
	public boolean resolveFunctions(Function<String, FunctionDefinition> definedFunctions,
			Consumer<String> errorHandler) {
		return fields.stream().filter(field -> field.second().resolveFunctions(definedFunctions, errorHandler))
				.count() == fields.size();
	}
	...
	public boolean typeCheck(Consumer<String> errorHandler) {
		...
		if (fieldCounts.entrySet().stream()//
				...
				.peek(e -> errorHandler.accept(String.format("%d:%d: Field %s is defined %d times.", line(), column(),
						e.getKey(), e.getValue())))//
				.count() > 0) {
			ok = false;
		}
		if (ok) {
			type = new Imyhat.ObjectImyhat(
