...
public abstract class SignatureVariable implements Target {
	 ...
	 * 
	 * This method operates different depending on the storage of the signature.
	 * <ul>
	 * <li>If the signature does not actually depend on the input
	 * ({@link SignatureStorage#STATIC_FIELD}), then the method passed will be a
	 * <tt>clinit</tt> method and the constant value should be pushed on the stack.
	 * <li>If the signature depends on the input
	 * ({@link SignatureStorage#STATIC_METHOD}), then the method passed will be a
	 * method that takes one input, the stream variable to be signed, and the
	 * resulting value should be pushed on the stack.
	 * </ul>
	 * 
	 * @param method
	 *            the method in which to generate the byte code
	 * @param streamType
