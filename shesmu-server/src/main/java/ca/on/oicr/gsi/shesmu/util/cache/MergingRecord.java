...
/**
 * Stores an incrementally appending list of items. When new items are provided,
 * old ones with matching IDs are replaced
 *
 * @param <V>
...
public final class MergingRecord<V, I> implements Record<Stream<V>> {
	 ...
	 */
	public static <V, I> BiFunction<Owner, Updater<Stream<V>>, Record<Stream<V>>> by(Function<V, I> getId) {
	...
	public MergingRecord(Owner owner, Updater<Stream<V>> fetcher, Function<V, I> getId) {
		...
		List<V> value;
		try {
			value = Optional.of(fetcher.update(fetchTime)).orElse(Stream.empty()).collect(Collectors.toList());
	...
	public synchronized Stream<V> refresh() {
		...
		if (Duration.between(fetchTime, now).toMinutes() > owner.ttl()) {
			try (AutoCloseable timer = refreshLatency.start(owner.name())){
				final List<V> buffer = Optional.ofNullable(fetcher.update(fetchTime)).orElse(Stream.empty())
						.collect(Collectors.toList());
				final Set<I> newIds = buffer.stream().map(getId).collect(Collectors.toSet());
				value = Stream.concat(//
						value.stream()//
								.filter(item -> !newIds.contains(getId.apply(item))), //
						buffer.stream())//
						.collect(Collectors.toList());
